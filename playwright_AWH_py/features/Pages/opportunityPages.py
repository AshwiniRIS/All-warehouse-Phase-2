...
class opportunityPages:
    ...
    async def navigateToOpp(self):
        ...
        await searchButton.fill(opportunityPages.enquiry_name)
        await searchButton.press("Enter")
        print("search is able to use")
        await self.page.get_by_role("link", name=opportunityPages.enquiry_name).first.click()
    ...
    async def generateProposal(self):
        ...
        # await self.page.locator("//input[@name='Save_Proposal_as']").fill(opportunityPages.enquiry_name)
        enquiry_name = shared.get("enquiry_name")
        await self.page.locator("//input[@name='clientName']").fill(enquiry_name)
        ...
        pdftext = enquiry_name + "_Parrys_1000.Pdf"
